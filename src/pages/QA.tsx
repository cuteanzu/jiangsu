...
const Page = styled.div`
  ...
  background:
    radial-gradient(ellipse at 78% 0%, rgba(225,170,120,0.15) 0%, transparent 46%),
    radial-gradient(ellipse at 12% 48%, rgba(200,145,95,0.10) 0%, transparent 40%),
...
const ROLE_QA_GUIDES: Record<AudienceRole, {
  ...
  gaokao: {
    title: "优先看择校、城市和政策类问题",
    description: "默认把学校层次、城市选择、转专业、中外合作和毕业去向相关问答排在前面。",
    categories: ["freshman", "study", "city-life", "career"],
  },
  freshman: {
    title: "优先看入学生活和校园适应问题",
    description: "默认把宿舍、生活费、转专业和城市适应相关问答排在前面。",
    categories: ["dorm", "city-life", "study", "freshman"],
  },
  college: {
    title: "优先看成长路径和发展问题",
    description: "默认把考研保研、就业去向、转专业和城市机会相关问答排在前面。",
    categories: ["exam", "career", "study", "city-life"],
  },
};

function getCategoryMeta(category: string | "all") {
  if (category === "all") return { label: "推荐", color: "#b07242" };
  return CATEGORY_META[category as PostCategory] ?? { label: "问答", color: "#4a8eb5" };
}
...
export default function QA() {
  ...
  const handleItemKeyDown = (event: React.KeyboardEvent<HTMLDivElement>, id: string) => {
      ...
      <Container>
        <Hero>
          <HeroTop>
            <TitleBlock>
              ...
              <Subtitle>把择校、入学、学习和发展里的关键问题，交给真实经历来回答。</Subtitle>
            </TitleBlock>
            <SearchBox>
              <SearchIcon />
              <input
        ...
        {featured.length > 0 && (
          <Section>
            <SectionHead>
              <div>
                <SectionTitle>精选问答</SectionTitle>
            ...
            <FeaturedGrid>
              <FeaturedMain type="button" onClick={() => toggleExpand(featured[0].id)}>
                <div>
                  <Tag $color={getCategoryMeta(featured[0].category).color}>
                    {getCategoryMeta(featured[0].category).label}
                  </Tag>
                  <FeaturedTitle $large>{featured[0].question}</FeaturedTitle>
                  <Excerpt>{featured[0].answer}</Excerpt>
                </div>
                ...
                {featured.slice(1, 3).map((item) => (
                  <FeaturedSmall key={item.id} type="button" onClick={() => toggleExpand(item.id)}>
                    <div>
                      ...
                      <FeaturedTitle>{item.question}</FeaturedTitle>
                    </div>
                    {renderMeta(item)}
                  </FeaturedSmall>
                ))}
        ...
        <Section>
          <BodyGrid>
            <div>
              <SectionHead>
                <div>
                  <SectionTitle>全部问答</SectionTitle>
                  <SectionNote>点开问题查看完整回答，筛选和身份视角会保留。</SectionNote>
                </div>
              </SectionHead>
                      ...
                      >
                        <QuestionRow>
                          ...
                          <div>
                            ...
                            {!expanded && <AnswerPreview>{item.answer}</AnswerPreview>}
                            {expanded && <Answer>{item.answer}</Answer>}
                            {renderMeta(item)}
                          </div>
